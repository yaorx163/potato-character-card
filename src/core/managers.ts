...
class 黑市管理器 {
  ...
  购买商品(商品名: string, 购买数量: number, 目标: 可目标实体 | 母畜实体 | null = null): 购买结果 {
    ...
    // 计算总价
    const 总价 = 配置.价格 * 购买数量;
    const 当前金币 = this.游戏总控.资源管理.获取金币();
    if (当前金币 < 总价) {
      return { 成功: false, 原因: `金币不足，需要 ${总价}，当前 ${当前金币}` };
    }

    // 扣除金币
    this.游戏总控.资源管理.修改金币(-总价);
  ...
  购买奴隶(商品ID: string): 奴隶购买结果 {
    ...
    const 当前金币 = this.游戏总控.资源管理.获取金币();
    if (当前金币 < 奴隶.价格) {
      return { 成功: false, 原因: `金币不足，需要 ${奴隶.价格}，当前 ${当前金币}` };
    }

    // 扣除金币
    this.游戏总控.资源管理.修改金币(-奴隶.价格);
