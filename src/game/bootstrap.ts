...
import { 初始化任务法术市场 } from '../data/rules';
import {
    运行时配置,
    游戏初始配置,
    初始地点配置,
...
class 游戏引导器 {
    ...
    启动新游戏(): 游戏总控 {
        ...
        初始化任务法术市场(
            工厂,
            this.游戏实例.系统管理.获取任务管理器(),
            this.游戏实例.系统管理.获取黑市管理器(),
            this.游戏实例.系统管理.获取法术管理器(),
        );
        // 5. 生成初始可袭击地点
        this.生成初始地点(工厂);
        this.生成初始实体(工厂);
    ...
    private 生成初始实体(工厂: 工厂管理器): void{
        ...
        for (let i = 0; i < 游戏初始配置.初始母畜数; i++) {
            ...
            this.游戏实例.母畜管理.添加母畜(母畜);

        }
        for (let i = 0; i < 游戏初始配置.初始冠军数; i++) {
            const 随机母畜 = this.游戏实例.系统管理.获取实体管理器().获取所有母畜()[Math.floor(Math.random() * 游戏初始配置.初始母畜数)]!;
            if (!随机母畜) continue;
            const 冠军 = 工厂.从母畜生育哥布林冠军(随机母畜, {});
            this.游戏实例.冠军管理.添加冠军(冠军);
        }
